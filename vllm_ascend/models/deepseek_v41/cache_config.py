...
def get_layer_tuples(specs):
    ...
    state = sorted((name for name, spec in specs.items() if isinstance(spec, CIRCULAR_BUFFER_SPECS)), key=_layer_number)
...
def group_cache_specs(specs):
    ...
    state_names = [name for name, spec in padded.items() if isinstance(spec, CIRCULAR_BUFFER_SPECS)]
    groups = [
