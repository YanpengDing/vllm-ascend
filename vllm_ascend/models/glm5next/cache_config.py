...
Physical storage is packed into one block-major allocation with two page-size
...
def get_glm5_kv_cache_config(
    ...
    """Pack all GLM-Next caches into one block-major backing allocation."""
    ...
    backing_size = block_stride * num_blocks
    tensors: list[KVCacheTensor] = []

    # A descriptor identifies one strided page slot. Layers in independent
    # scheduler groups may share it because they allocate disjoint block IDs.
    for slot in range(layout.main_slot_count):
        ...
        tensors.append(
            KVCacheTensor(
                size=backing_size,
                shared_by=shared_by,
                offset=slot * layout.main_page_size,
                block_stride=block_stride,
            )
        )

    small_base = layout.main_slot_count * layout.main_page_size
    for slot, (indexer_name, state_name) in enumerate(
        zip(layout.indexer_names, layout.state_names)
    ):
        tensors.append(
            KVCacheTensor(
                size=backing_size,
                shared_by=[indexer_name, state_name],
                offset=small_base + slot * layout.small_page_size,
                block_stride=block_stride,
            )
        )
...
def get_glm5_max_memory_usage(
    ...
    """Return pool capacity needed by all independently allocating groups."""

    if _get_glm5_cache_layout(groups) is None:
        raise ValueError("Expected GLM-Next cache groups.")
    blocks = sum(
        cdiv(
