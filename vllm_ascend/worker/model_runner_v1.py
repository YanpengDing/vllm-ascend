...
class NPUModelRunner(GPUModelRunner):
    ...
    def _allocate_kv_cache_tensors(self, kv_cache_config: KVCacheConfig) -> dict[str, torch.Tensor]:
        ...
        if independent_descriptors:
            self.hybrid_with_attn_and_mamba = False
            if len(independent_descriptors) != len(
                kv_cache_config.kv_cache_tensors
            ):
                raise ValueError(
                    "Independent and shared-backing KV cache descriptors "
                    "cannot be mixed in one cache plan."
                )

            for descriptor in independent_descriptors:
                if descriptor.block_stride <= 0:
                    raise ValueError(
                        "An independent cache descriptor requires a positive "
                        "block "
                        f"stride, got {descriptor.block_stride}."
                    )
                if descriptor.size != (
                ...
                if descriptor.offset != 0:
                    raise ValueError(
                        "Independent cache descriptors must start at "
                        f"offset zero, got {descriptor.offset}."
                    )
                ...
                for layer_name in descriptor.shared_by:
                    ...
                    if layer_name in kv_cache_raw_tensors:
                        raise ValueError(
                            f"Cache layer {layer_name!r} has multiple independent "
    ...
    @staticmethod
    def _reshape_independent_cache_tensor(
        raw_tensor: torch.Tensor,
        ...
        """Create a layer view over an independent block-strided cache."""
    ...
    def _reshape_kv_cache_tensors(
        ...
        for descriptor in kv_cache_config.kv_cache_tensors:
            if isinstance(descriptor, AscendIndependentKVCacheTensor):
                for layer_name in descriptor.shared_by:
                    if layer_name in independent_descriptors:
                        raise ValueError(
                            f"Cache layer {layer_name!r} has multiple independent "
                            "physical descriptors."
                        )
                    independent_descriptors[layer_name] = descriptor
        ...
        for group in self._kv_cache_spec_attn_group_iterator():
            ...
            for layer_name in group.layer_names:
                ...
                if layer_name in independent_descriptors:
                    descriptor = independent_descriptors[layer_name]
                    raw_tensor = kv_cache_raw_tensors[layer_name]
                    if not isinstance(raw_tensor, torch.Tensor):
                        raise TypeError(
                            "Independent cache layers require one raw backing "
                    ...
                    if isinstance(current_kv_cache_spec, MambaSpec):
                        ...
                        for shape, dtype in zip(
                            ...
                            state_tensors.append(
                                self._reshape_independent_cache_tensor(
                                    raw_tensor,
                                    target_shape,
                                    dtype,
                                    descriptor.block_stride,
                    ...
                    if not isinstance(current_kv_cache_spec, AttentionSpec):
                        raise TypeError(
                            "Unsupported independent cache spec for "
                    ...
                    is_split_mla_cache = (
                        ...
                        and current_kv_cache_spec.indexes_kv_by_block_stride
                        and current_kv_cache_spec.compress_ratio == 1
                    )
                    if is_split_mla_cache:
                        ...
                        if (
                            k_payload_size + v_payload_size
                            > current_kv_cache_spec.page_size_bytes
                        ):
                            raise ValueError(
                                "Split MLA payload exceeds its independent physical "
                                f"page for {layer_name}."
                            )
                    ...
                    # Keep one scheduler block in dimension 0 so any padding
                    # in the physical page remains outside the kernel view.
                    ...
                    if is_quantized_indexer:
                        ...
                        if (
                            ...
                            raise ValueError(
                                "Quantized indexer payload exceeds its independent "
            ...
            if (isinstance(attn_module, Attention)
                ...
                if self.use_sparse:
                    ...
                    kv_cache_spec[layer_name] = AscendMLAAttentionSpec(
                        ...
                        # Preserve the layout contract published by the
                        # attention layer. The runner adapts the spec to an
                        # Ascend type, but must not infer model capabilities
                        # by inspecting the top-level model configuration.
