...
def test_groups_share_block_ids_and_pack_two_page_classes(pool):
    ...
    assert {tensor.size for tensor in plan.kv_cache_tensors} == {budget}
    ...
    assert indexer.offset == layout.main_page_size
    ...
    # Groups sharing a physical slot still allocate distinct global block IDs.
    required_blocks = sum(
        cdiv(
    ...
    assert required_blocks > groups[0].kv_cache_spec.max_memory_usage_pages(
        config
    )
...
def test_pipeline_projection_supports_a_mamba_only_worker():
    config = make_config()
    groups = get_glm5_kv_cache_groups(config, make_specs())
    ...
    layout = _get_glm5_cache_layout(projected_groups)
    assert layout is not None
